    ...
    protected function parseResults($html)
    {
        ...
        $departure
            ...
            ->reduce(function (Crawler $node, $i) use(&$results) {
                ...
                $result->setOrigin($node->filter('.avail-table-detail-table')->eq(0)->filter('.avail-table-detail .text-center div')->eq(1)->text());
                ...
                $result->setDestination($node->filter('.avail-table-detail-table')->eq(0)->filter('.avail-table-detail')->eq(1)->filter('.text-center div')->eq(1)->text());
